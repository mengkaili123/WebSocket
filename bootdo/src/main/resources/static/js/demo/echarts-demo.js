$(function () {
    ...
    var xData = [20, 30, 5.0, 10, 60, 20, 30];
    var baroption = {
        toolbox: {
            feature: {
        ...
        xAxis : [
            {
                type : 'category',
                data : ['温度','储油罐温度','电压','传送带速度','储油量','电机转速','冷风机转速']
            }
        ],
        yAxis : [
            {
        ...
        series : [
            ...
            },
            {
                ...
                type:'bar',
                data:[],
                // data:[20, 30, 5.0, 10, 60, 20, 30],
                // markPoint : {
                //     data : [
                //         {name : '年最高温度', value : 90, xAxis: 7, yAxis: 183, symbolSize:18},
                //         {name : '年最低温度', value : 10, xAxis: 11, yAxis: 3}
                //     ]
                // },
                // markLine : {
                //     data : [
    ...
    function loadChartOut1() {
        $.getJSON('/system/weather3/list', function (data) {
            ...
            for (var i = 0; i < data.rows.length; i++) {
                rowsData1.push(data.rows[i].temperature);
                rowsData1.push(data.rows[i].temperature1);
                rowsData1.push(data.rows[i].dianya);
                rowsData1.push(data.rows[i].speed);
                rowsData1.push(data.rows[i].yeya);
                rowsData1.push(data.rows[i].zhuansu);
                rowsData1.push(data.rows[i].zhuansu1);
            }
    ...
    function loadChartOut() {
        $.getJSON('/system/weather/list', function (data) {
            //上面这个url是重点，组成为：项目名/过滤前缀/Controller中mapping值，当页面加载此js时，它会向后台取数据
            // if (data.success1) {
            //     barChart.showLoading({text: '正在努力的读取数据中...'});
            console.log(JSON.stringify(data))
            var rowsData = [];
            for (var i = 0; i < data.rows.length; i++) {
                rowsData.push(data.rows[i].temperature);
                rowsData.push(data.rows[i].temperature1);
                rowsData.push(data.rows[i].dianya);
                rowsData.push(data.rows[i].speed);
                rowsData.push(data.rows[i].yeya);
                rowsData.push(data.rows[i].zhuansu);
                rowsData.push(data.rows[i].zhuansu1);
            }
            // console.log("data"+rows)
                barChart.setOption({
                        series: [
